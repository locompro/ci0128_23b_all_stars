...
[TestFixture]
public class FindPriceAnomaliesTest
{
    /// <summary>
    ...
    public void SetUp()
    {
        ...
        _mockAnomalyDetectionService = new Mock<IAnomalyDetectionService>();

        // Mock the service scope to return the mock moderation service
        _mockServiceScope = new Mock<IServiceScope>();
        _mockServiceScope.Setup(x => x.ServiceProvider.GetService(typeof(IAnomalyDetectionService)))
            .Returns(_mockAnomalyDetectionService.Object);
    ...
    private Mock<IAnomalyDetectionService> _mockAnomalyDetectionService;
    ...
    public async Task ExecuteAsync_CallsAssignPossibleModeratorsAsync()
    {
        ...
        var task = new FindPriceAnomaliesTask(_mockServiceProvider.Object);

        // Act
        await task.ExecuteAsync(cancellationToken);

        // Assert
        _mockAnomalyDetectionService.Verify(service => service.FindPriceAnomaliesAsync(), Times.Once);
    }
}